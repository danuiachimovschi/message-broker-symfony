...
class Connection
{
    ...
    public function setup(): void
    {
        if (!array_key_exists(self::BROKERS_LIST, $this->kafkaConfig)) {
            throw new LogicException(sprintf('The "%s" option is required for the Kafka Messenger transport "%s".', self::BROKERS_LIST, $this->transportName));
        }
        ...
        if (
            ...
            throw new LogicException(sprintf('At least one of "%s" or "%s" options is required for the Kafka Messenger transport "%s".', self::CONSUMER_TOPICS_NAME, self::PRODUCER_TOPIC_NAME, $this->transportName));
        }
    }
    ...
    public function get(): Message
    {
        if (!array_key_exists(self::GROUP_ID, $this->kafkaConfig)) {
            throw new LogicException(sprintf('The transport "%s" is not configured to consume messages because "%s" option is missing.', $this->transportName, self::GROUP_ID));
        }
    ...
    private static function optionsValidator(array $options, string $transportName): void
    {
        ...
        if (0 < \count($invalidOptions)) {
            throw new LogicException(sprintf('Invalid option(s) "%s" passed to the Kafka Messenger transport "%s".', implode('", "', $invalidOptions), $transportName));
        }
    }
    ...
    private function getTopics(): array
    {
        if (!array_key_exists(self::CONSUMER_TOPICS_NAME, $this->kafkaConfig)) {
            throw new LogicException(sprintf('The transport "%s" is not configured to consume messages because "%s" option is missing.', $this->transportName, self::CONSUMER_TOPICS_NAME));
        }

        if (!is_array($this->kafkaConfig[self::CONSUMER_TOPICS_NAME])) {
            throw new LogicException(sprintf('The "%s" option type must be array, %s given in "%s" transport.', self::CONSUMER_TOPICS_NAME, gettype($this->kafkaConfig[self::CONSUMER_TOPICS_NAME]), $this->transportName));
        }
    ...
    private function getConsumerConsumeTimeout(): int
    {
        ...
        if (!is_int($this->kafkaConfig[self::CONSUMER_CONSUME_TIMEOUT_MS])) {
            throw new LogicException(sprintf('The "%s" option type must be integer, %s given in "%s" transport.', self::CONSUMER_CONSUME_TIMEOUT_MS, gettype($this->kafkaConfig[self::CONSUMER_CONSUME_TIMEOUT_MS]), $this->transportName));
        }
    ...
    private function getTopic(): string
    {
        if (!array_key_exists(self::PRODUCER_TOPIC_NAME, $this->kafkaConfig)) {
            throw new LogicException(sprintf('The transport "%s" is not configured to dispatch messages because "%s" option is missing.', $this->transportName, self::PRODUCER_TOPIC_NAME));
        }

        if (!is_string($this->kafkaConfig[self::PRODUCER_TOPIC_NAME])) {
            throw new LogicException(sprintf('The "%s" option type must be string, %s given in "%s" transport.', self::PRODUCER_TOPIC_NAME, gettype($this->kafkaConfig[self::PRODUCER_TOPIC_NAME]), $this->transportName));
        }
    ...
    private function getMessageFlags(): int
    {
        ...
        if (!is_bool($this->kafkaConfig[self::PRODUCER_MESSAGE_FLAGS_BLOCK])) {
            throw new LogicException(sprintf('The "%s" option type must be boolean, %s given in "%s" transport.', self::PRODUCER_MESSAGE_FLAGS_BLOCK, gettype($this->kafkaConfig[self::PRODUCER_MESSAGE_FLAGS_BLOCK]), $this->transportName));
        }
    ...
    private function getPartitionId(): int
    {
        ...
        if (!is_int($this->kafkaConfig[self::PRODUCER_PARTITION_ID_ASSIGNMENT])) {
            throw new LogicException(sprintf('The "%s" option type must be integer, %s given in "%s" transport.', self::PRODUCER_PARTITION_ID_ASSIGNMENT, gettype($this->kafkaConfig[self::PRODUCER_PARTITION_ID_ASSIGNMENT]), $this->transportName));
        }
    ...
    private function getProducerPollTimeout(): int
    {
        ...
        if (!is_int($this->kafkaConfig[self::PRODUCER_POLL_TIMEOUT_MS])) {
            throw new LogicException(sprintf('The "%s" option type must be integer, %s given in "%s" transport.', self::PRODUCER_POLL_TIMEOUT_MS, gettype($this->kafkaConfig[self::PRODUCER_POLL_TIMEOUT_MS]), $this->transportName));
        }
    ...
    private function getProducerFlushTimeout(): int
    {
        ...
        if (!is_int($this->kafkaConfig[self::PRODUCER_FLUSH_TIMEOUT_MS])) {
            throw new LogicException(sprintf('The "%s" option type must be integer, %s given in "%s" transport.', self::PRODUCER_FLUSH_TIMEOUT_MS, gettype($this->kafkaConfig[self::PRODUCER_FLUSH_TIMEOUT_MS]), $this->transportName));
        }
